...
int main(int argc, char **argv)
{
    ...
    int
        *hst_mtxA, *hst_mtx_output,
        *dev_mtxB, *dev_mtx_output;
    ...
    cudaMemcpy(dev_mtxB, hst_mtxA, mtxSize * sizeof(int), cudaMemcpyHostToDevice);
    ...
    // launch the kernel
    displacer_matrix<<<blocks, threads>>>(dev_mtxB, dev_mtx_output, MTX_COL_SIZE, MTX_ROW_SIZE);
    ...
    cudaMemcpy(hst_mtx_output, dev_mtx_output, mtxSize * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    // print the matrix
    print_int_matrix((char*)"> Original matrix", hst_mtxA, mtxSize, MTX_COL_SIZE);
    print_int_matrix((char*)"> Displaced matrix", hst_mtx_output, mtxSize, MTX_COL_SIZE);
